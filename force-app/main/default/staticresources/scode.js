...
var SCCodeVersion="SC24.3"
////////////////////////////////////////////////////////////////////////////////////////////////
// Development Report Suite - change to "informatica-mysupport-prod" when move to PRODUCTION  //
////////////////////////////////////////////////////////////////////////////////////////////////

var s_account1="informatica-mysupport-prod"
...
s.linkInternalFilters="javascript:,my.informatica.com,.informatica."
//s.linkInternalFilters="javascript:,communities-test.informatica.com,communities.informatica.com,PSV23CMSMAP1,PSV23CMSMAP2"
...
s.campaign=s.getValOnce(s.getQueryParam('ENTER QUERY PARAMETERS HERE'),'s_campaign',30);	/* CHANGE TO QUERY STRING ID THAT IS USED. eg... (www.....?utm_source=12345)  */

// Internal Campaigns: 
s.eVar4=s.getValOnce(s.getQueryParam('ENTER QUERY PARAMETERS HERE'),'int_campaign',30);	    /* CHANGE TO QUERY STRING ID THAT IS USED. eg... (www.....?utm_source=12345)  */
...
if(s.prop6)
{
	...
	{
		s.prop8='zero';
		s.events=s.apl(s.events,'event6',',',2); //Setting Null searches event
	}
}
...
//Search Originating Page
s.prop14=s.getPreviousValue(s.pageName,'gpv_p14','event5'); 

// Page URL excluding Query Strings
s.prop20=location.protocol + "//" + location.host + location.pathname	

//HARD CODED: [Sitecatalyst code version + internal version number]
	// Set at top of this file
	s.prop21=SCCodeVersion;

/* Previous page value and getPercentPageViewed */
 s.prop17=s.getPreviousValue(s.pageName,'gpv2','');
	if (s.prop17)
...
// getTimeParting without year check
	s.eVar18=s.getTimeParting('w','-8') + ":" + s.getTimeParting('d','-8') + ':' + s.getTimeParting('h','-8') ; // Set week , day , hour	
	if(s.eVar18 && s.eVar18!=""){
	s.prop19=s.eVar18;
	}
	
/* Visit Number */
	s.eVar17=s.getVisitNum();
	if(s.eVar17 && s.eVar17!=""){
	s.prop23=s.eVar17;
	}
...
/* Pathing per KB Search */
	if(s.eVar6 && s.eVar6!=""){
	s.prop15=s.eVar6 + ":" + s.pageName
	}	

// Time To Resolve Call
	if (s.events){   
			  if (s.events.match(/event1/i)) s.prop13='start';
			  if (s.events.match(/event2/i)) s.prop13='stop';			 
				 s.eVar10=s.getTimeToComplete(s.prop13,'ttcr',0); // Time to Resolve Call
	}
}
...
/*
 *Time to Build Cart: ver. 0.4 - return the time from start to stop 
 */
...
s.trackingServer="informatica.112.2o7.net"
